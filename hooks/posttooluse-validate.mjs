...
function runChainInjection(fileContent, matchedSkills, chainMap, sessionId, pluginRoot, logger, env = process.env, skillStore, projectRoot) {
  const l = logger || log;
  ...
  for (const { sourceSkill, rule } of uniqueCandidates) {
    ...
    const content = payload.mode === "body" && payload.body ? payload.body.trim() : [
    ...
    l.debug(
      payload.mode === "body" ? "posttooluse-chain-injected" : "posttooluse-chain-summary-fallback",
      {
        sourceSkill,
        targetSkill: rule.targetSkill,
        source: payload.source,
