...
import { parseSeenSkills, appendSeenSkill, parseLikelySkills, compileSkillPatterns, matchPathWithReason, matchBashWithReason, matchImportWithReason, rankEntries, } from "./patterns.mjs";
...
export function parseInput(raw, logger) {
    ...
    const toolTarget = toolName === "Bash"
        ? (toolInput.command || "")
        : (toolInput.file_path || "");
    l.debug("input-parsed", { toolName, sessionId: sessionId });
...
export function deduplicateSkills({ matchedEntries, matched, toolName, toolInput, injectedSkills, dedupOff, maxSkills, likelySkills, compiledSkills, setupMode }, logger) {
    ...
    if (setupModeActive) {
        ...
        if (!dedupOff && injectedSkills.has(SETUP_MODE_BOOTSTRAP_SKILL)) {
            ...
            bootstrapEntry.effectivePriority = Math.max((typeof basePriority === "number" ? basePriority : 0) + SETUP_MODE_PRIORITY_BOOST, maxPriority + 1);
        ...
        if (loaded.length > 0 && usedBytes + byteLen > budget) {
            ...
            if (summary) {
                ...
                if (usedBytes + summaryByteLen <= budget) {
                    ...
                    if (hasEnvDedup) {
                        process.env.VERCEL_PLUGIN_SEEN_SKILLS = appendSeenSkill(process.env.VERCEL_PLUGIN_SEEN_SKILLS, skill);
                    }
...
function run() {
    ...
    log.debug("dedup-strategy", { strategy: dedupStrategy, sessionId: sessionId, seenEnv });
    ...
    const injectedSkills = hasEnvDedup ? parseSeenSkills(seenEnv) : new Set();
    // Stage 3: matchSkills
