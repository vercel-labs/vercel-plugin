...
export function buildSkillMap(rootDir) {
    ...
    for (const skill of parsed) {
        ...
        if (meta.pathPatterns !== undefined) {
            ...
            addWarning(`skill "${skill.dir}": metadata.filePattern is deprecated, rename to pathPatterns`, {
        ...
        if (typeof rawPathPatterns === "string") {
            addWarning(`skill "${skill.dir}": metadata.pathPatterns is a string, coercing to array`, {
            ...
            addWarning(`skill "${skill.dir}": metadata.pathPatterns is not an array (${typeof rawPathPatterns}), defaulting to []`, {
                code: "INVALID_TYPE",
                skill: skill.dir,
                field: "pathPatterns",
        ...
        const filteredPathPatterns = pathPatterns.filter((p, i) => {
            if (typeof p !== "string") {
                addWarning(`skill "${skill.dir}": metadata.pathPatterns[${i}] is not a string (${typeof p}), removing`, {
            ...
            if (p === "") {
                addWarning(`skill "${skill.dir}": metadata.pathPatterns[${i}] is empty, removing`, {
                    code: "ENTRY_EMPTY",
                    skill: skill.dir,
                    field: `pathPatterns[${i}]`,
                    valueType: "string",
        ...
        if (meta.bashPatterns !== undefined) {
            ...
            addWarning(`skill "${skill.dir}": metadata.bashPattern is deprecated, rename to bashPatterns`, {
                code: "DEPRECATED_FIELD",
                skill: skill.dir,
                field: "bashPattern",
        ...
        if (typeof rawBashPatterns === "string") {
            addWarning(`skill "${skill.dir}": metadata.bashPatterns is a string, coercing to array`, {
                code: "COERCE_STRING_TO_ARRAY",
                skill: skill.dir,
                field: "bashPatterns",
                valueType: "string",
            ...
            addWarning(`skill "${skill.dir}": metadata.bashPatterns is not an array (${typeof rawBashPatterns}), defaulting to []`, {
                code: "INVALID_TYPE",
                skill: skill.dir,
                field: "bashPatterns",
        ...
        // Filter out non-string and empty-string entries
        const filteredBashPatterns = bashPatterns.filter((p, i) => {
            if (typeof p !== "string") {
                addWarning(`skill "${skill.dir}": metadata.bashPatterns[${i}] is not a string (${typeof p}), removing`, {
                    code: "ENTRY_NOT_STRING",
                    skill: skill.dir,
            ...
            if (p === "") {
                addWarning(`skill "${skill.dir}": metadata.bashPatterns[${i}] is empty, removing`, {
                    code: "ENTRY_EMPTY",
                    skill: skill.dir,
                    field: `bashPatterns[${i}]`,
                    valueType: "string",
        ...
        let rawImportPatterns = meta.importPatterns !== undefined ? meta.importPatterns : [];
        let importPatterns;
        if (typeof rawImportPatterns === "string") {
            addWarning(`skill "${skill.dir}": metadata.importPatterns is a string, coercing to array`, {
                code: "COERCE_STRING_TO_ARRAY",
                skill: skill.dir,
            ...
            addWarning(`skill "${skill.dir}": metadata.importPatterns is not an array (${typeof rawImportPatterns}), defaulting to []`, {
                code: "INVALID_TYPE",
                skill: skill.dir,
                field: "importPatterns",
        ...
        const filteredImportPatterns = importPatterns.filter((p, i) => {
            if (typeof p !== "string") {
                addWarning(`skill "${skill.dir}": metadata.importPatterns[${i}] is not a string (${typeof p}), removing`, {
                    code: "ENTRY_NOT_STRING",
                    skill: skill.dir,
            ...
            if (p === "") {
                addWarning(`skill "${skill.dir}": metadata.importPatterns[${i}] is empty, removing`, {
                    code: "ENTRY_EMPTY",
                    skill: skill.dir,
                    field: `importPatterns[${i}]`,
                    valueType: "string",
    ...
    for (const [skill, config] of Object.entries(skills)) {
        ...
        if ("priority" in cfg) {
            ...
            if (typeof p !== "number" || Number.isNaN(p)) {
                addWarning(`skill "${skill}": priority is not a valid number, defaulting to 5`, {
        ...
        if ("pathPatterns" in cfg) {
            if (!Array.isArray(cfg.pathPatterns)) {
                addWarning(`skill "${skill}": pathPatterns is not an array, defaulting to []`, {
                    code: "INVALID_TYPE",
                    skill,
                    field: "pathPatterns",
                ...
                pathPatterns = cfg.pathPatterns.filter((p, i) => {
                    if (typeof p !== "string") {
                        addWarning(`skill "${skill}": pathPatterns[${i}] is not a string, removing`, {
                            code: "ENTRY_NOT_STRING",
                            skill,
                            field: `pathPatterns[${i}]`,
                            valueType: typeof p,
                    ...
                    if (p === "") {
                        addWarning(`skill "${skill}": pathPatterns[${i}] is empty, removing`, {
                            code: "ENTRY_EMPTY",
                            skill,
                            field: `pathPatterns[${i}]`,
                            valueType: "string",
        ...
        if ("bashPatterns" in cfg) {
            if (!Array.isArray(cfg.bashPatterns)) {
                addWarning(`skill "${skill}": bashPatterns is not an array, defaulting to []`, {
                    code: "INVALID_TYPE",
                    skill,
                    field: "bashPatterns",
                ...
                bashPatterns = cfg.bashPatterns.filter((p, i) => {
                    if (typeof p !== "string") {
                        addWarning(`skill "${skill}": bashPatterns[${i}] is not a string, removing`, {
                            code: "ENTRY_NOT_STRING",
                            skill,
                            field: `bashPatterns[${i}]`,
                            valueType: typeof p,
                    ...
                    if (p === "") {
                        addWarning(`skill "${skill}": bashPatterns[${i}] is empty, removing`, {
                            code: "ENTRY_EMPTY",
                            skill,
                            field: `bashPatterns[${i}]`,
                            valueType: "string",
        ...
        if ("importPatterns" in cfg) {
            if (!Array.isArray(cfg.importPatterns)) {
                addWarning(`skill "${skill}": importPatterns is not an array, defaulting to []`, {
                    code: "INVALID_TYPE",
                    skill,
                    field: "importPatterns",
                ...
                importPatterns = cfg.importPatterns.filter((p, i) => {
                    if (typeof p !== "string") {
                        addWarning(`skill "${skill}": importPatterns[${i}] is not a string, removing`, {
                            code: "ENTRY_NOT_STRING",
                            skill,
                            field: `importPatterns[${i}]`,
                            valueType: typeof p,
                    ...
                    if (p === "") {
                        addWarning(`skill "${skill}": importPatterns[${i}] is empty, removing`, {
                            code: "ENTRY_EMPTY",
                            skill,
                            field: `importPatterns[${i}]`,
                            valueType: "string",
